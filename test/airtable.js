...
import pullTable from "../src/pull-table-from-airtable"
import updateOKRecordsToAirtable from "../src/update-ok-records-to-airtable"
...
test.beforeEach(async (t) => {
  ...
  const output_dirname = path.join(local_save_path, "test_a")
...
test.serial("run initial pull", async(t) => {
  ...
  let output_data = await fs.readFile(output_filename, 'utf-8')
...
test.serial("push changed table", async (t) => {
  ...
  await fs.writeFile(diff_filename, f6_text, "utf-8")
  ...
  let output_data = await fs.readFile(diff_filename, "utf-8")
  output_data = JSON.parse(output_data)
